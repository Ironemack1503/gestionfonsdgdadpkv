  ...
  try {
    ...
    switch (action) {
      ...
      case 'create': {
        ...
        const { date, solde_initial, total_recettes, total_depenses, solde_final } = params;
        ...
        const { data, error } = await supabase
          ...
          .insert({
            ...
            total_depenses: total_depenses || 0,
            solde_final: solde_final || null,
            user_id: user.id,
          })
          .select()
          .single();
      ...
      case 'close': {
        ...
        if (oldData) {
          await supabase.from('audit_logs').insert({
            ...
            old_data: oldData,
            new_data: data,
            changed_fields: ['is_closed', 'closed_at', 'closed_by', 'solde_final'],
            user_id: user.id,
            user_email: user.username,
          });
        }
